...
où $x$ représente la position, $\dot{x}$ la vitesse de déplacement, $\theta$ l'angle d'inclinaison et $\dot{\theta}$ la vitesse angulaire initiale.
...
La figure \ref{fig:max_speed} présente l'évolution de la vitesse mesurée au cours de cet essai. La vitesse augmente jusqu'à atteindre une valeur maximale de $0.864\,\mathrm{m/s}$, puis reste limitée à cette valeur.
...
Cet essai a pour objectif d'observer l'influence d'une accélération linéaire sur la mesure de l'angle fournie par l'\acrshort{imu}. Pour cela, une perturbation manuelle est appliquée au robot en le poussant successivement dans les directions positive et négative.

Lors de l'application de la poussée, une variation rapide de l'angle mesuré est observée dans une direction opposée au sens du déplacement imposé. Cette variation est suivie d'un retour vers la valeur de consigne, puis d'une évolution transitoire dans le sens de la poussée avant que l'angle ne converge à nouveau vers sa valeur initiale.

Pour la perturbation positive (\ref{fig:hold_positive_angle}, \ref{fig:hold_positive_angle}), on constate un temps de régulation de 0.62 s
Pour la perturbation négative (\ref{fig:hold_negative_angle}, \ref{fig:hold_negative_angle}), on constate un temps de régulation de 0.72 s

Les essais réalisés pour les deux sens de déplacement présentent une réponse symétrique : l'évolution temporelle reste identique, seul le signe de la variation angulaire est inversé.
...
L'erreur obtenue reste limitée, ce qui confirme la capacité du système à suivre une consigne de déplacement.
...
On observe que l'angle mesuré par l'IMU suit plus fidèlement l'évolution de l'angle théorique obtenu à partir du modèle physique que celle de l'angle issu de la simulation de l'IMU.
...
La mesure d'autonomie peut être approximée à l'aide de la mesure de consommation effectuée plus haut.
