...
L'intégralité du code source est disponible dans le dossier compressé fourni sur GAPS ainsi que sur le depôt GIT.
...
Les différentes tâches de l'initialisation sont réalisées de manière séquentielle dans l'ordre suivant :
...
La bibliothèque inclue entre autre la création d'une classe TMC2226 ainsi que la gestion de l'UART bi-directionnel.
...
La bibliothèque développée prend en charge l'adresse (voir \ref{chap:electronique}.) lors de la construction de chaque instance de la classe \texttt{TMC2226}, permettant ainsi d'envoyer les commandes au driver concerné sans conflit avec les autres périphériques présents sur le bus.
